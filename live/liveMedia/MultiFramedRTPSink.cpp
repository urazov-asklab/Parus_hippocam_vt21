...
#define RTSP_NUM_VIDEO_DATA			(2*1000)
#define RTSP_NUM_VIDEO_MAX_DATA		(2.4*1000)

#define RTSP_NUM_AUDIO_DATA			(1*1000)
#define RTSP_NUM_AUDIO_MAX_DATA		(1.2*1000)
...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  {
      // Audio
      setPacketSizes(RTSP_NUM_AUDIO_DATA, RTSP_NUM_AUDIO_MAX_DATA);
  }
  else
  {
      // Video
      setPacketSizes(RTSP_NUM_VIDEO_DATA, RTSP_NUM_VIDEO_MAX_DATA);
  }
